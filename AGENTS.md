...
- If PSP work is involved, install the PSP driver (`\\.\AmdBcPsp`) before the GPU driver (`\\.\AMDBC250DreamV43`).
...
- READ_REG/WRITE_REG use direct GPU BAR5 (`DreamV3ReadRegister`/`DreamV3WriteRegister`), not the PSP proxy. The PSP proxy is for PSP-specific work only.
