...
private:
    ...
    // BC-250: BAR5 (MMIO) mapping for SMU/register access via Escape DDI.
    // BAR5 = 0xFE800000 (512KB) holds NBIO SMN bridge (0x38/0x3C) + GC registers.
    PVOID  m_Bar5Va;
    PHYSICAL_ADDRESS m_Bar5Pa;
    ULONG  m_Bar5Size;

public:
    // BC-250: Friend declaration for headless StartDevice
    friend NTSTATUS BddDdiStartDevice(
...
#pragma code_seg(pop)
    ...
    // BC-250: Escape DDI - SMU/register access from user-mode via D3DKMTEscape.
    // Handled directly in bdd_ddi.cxx (BddDdiEscape) using the helpers below.

    // BC-250: Map BAR5 MMIO in StartDevice. Returns STATUS_SUCCESS even on
    // failure (display must keep working without SMU access).
    NTSTATUS MapBar5();
    VOID UnmapBar5();

    // BC-250: BAR5 register access helpers (Non-Paged)
    ULONG ReadReg(ULONG Offset);
    VOID  WriteReg(ULONG Offset, ULONG Value);
    ULONG SmnRead(ULONG SmnAddr);
    VOID  SmnWrite(ULONG SmnAddr, ULONG Value);
    ULONG SmuQuery(USHORT Msg);
    ULONG SmuQueryParam(USHORT Msg, ULONG Param);
    ULONG SmuSendQueue(ULONG Queue, USHORT Msg, ULONG Param);
    NTSTATUS CoreUnlock();
