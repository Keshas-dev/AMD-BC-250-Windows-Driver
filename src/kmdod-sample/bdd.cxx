                                           ...
                                           _Out_ ULONG*             pNumberOfChildren)
{
    ...
    // BC-250: Map BAR5 MMIO for SMU/register access via Escape DDI.
    // Failure is non-fatal (display works without SMU control).
    MapBar5();
...
//    Status = CheckHardware();
    ...
    // BC-250 headless support: Try to acquire POST display, but continue if none found.
    // This is a compute GPU that may not have been initialized during POST.
    ...
    {
        // No POST display available - running in headless mode.
        // Driver starts without an active framebuffer.
        DbgPrint("BC-250: No POST display, running headless. Status: 0x%08lX\n", Status);
    }
    m_Flags.DriverStarted = TRUE;

    // Mark framebuffer as active only if we got a valid POST display.
    // NOTE: FrameBuffer.Ptr is NOT yet mapped here - it is mapped later in
    // SetSourceModeAndPath (via MapFrameBuffer) after CommitVidPn. We must not
    // write through FrameBuffer.Ptr while it is NULL, even if this flag is set.
    // BlackOutScreen() and PresentDisplayOnly() both guard on (Ptr != NULL),
    // which prevents a NULL-pointer BSOD (bugcheck 0x7E) if BlackOutScreen runs
    // (e.g. StopDeviceAndReleasePostDisplayOwnership on RDP/session switch)
    // before any CommitVidPn has mapped the framebuffer.
    if (NT_SUCCESS(Status) && m_CurrentModes[0].DispInfo.Width > 0)
    {
...
NTSTATUS BASIC_DISPLAY_DRIVER::MapBar5()
{
    PAGED_CODE();
    ...
    // BC-250 BAR5 physical base: 0xFE800000, size 512KB (0x80000).
    // Holds NBIO SMN bridge (0x38/0x3C) and GC registers (GC_BASE 0x1260 shift).
    m_Bar5Pa.QuadPart = 0xFE800000ULL;
    m_Bar5Size = 0x80000;

    m_Bar5Va = MmMapIoSpaceEx(m_Bar5Pa, m_Bar5Size, PAGE_READWRITE | PAGE_NOCACHE);
    if (m_Bar5Va == NULL)
    {
        DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL,
            "BC-250: MapBar5 - MmMapIoSpaceEx(0x%llX, 0x%X) FAILED\n",
            m_Bar5Pa.QuadPart, m_Bar5Size);
        return STATUS_DEVICE_NOT_READY;
    }

    DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL,
        "BC-250: MapBar5 - mapped PA=0x%llX size=0x%X VA=0x%p\n",
        m_Bar5Pa.QuadPart, m_Bar5Size, m_Bar5Va);

    return STATUS_SUCCESS;
}

VOID BASIC_DISPLAY_DRIVER::UnmapBar5()
{
    PAGED_CODE();

    if (m_Bar5Va != NULL)
    {
        MmUnmapIoSpace(m_Bar5Va, m_Bar5Size);
        m_Bar5Va = NULL;
    }
}

//
// BC-250: BAR5 register access. Only called via Escape at PASSIVE/DISPATCH.
// All reads/writes use volatile READ_REGISTER/WRITE_REGISTER intrinsics.
//

ULONG BASIC_DISPLAY_DRIVER::ReadReg(ULONG Offset)
{
    if (m_Bar5Va == NULL || Offset >= m_Bar5Size - sizeof(ULONG))
    {
        return 0xFFFFFFFF;
    }
    return READ_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + Offset));
}

VOID BASIC_DISPLAY_DRIVER::WriteReg(ULONG Offset, ULONG Value)
{
    if (m_Bar5Va == NULL || Offset >= m_Bar5Size - sizeof(ULONG))
    {
        return;
    }
    WRITE_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + Offset), Value);
}

//
// SMN access via NBIO BAR5+0x38/0x3C (Linux WREG32_PCIE/RREG32_PCIE path).
// Write SMN address to 0x38, then data to/from 0x3C.
//

ULONG BASIC_DISPLAY_DRIVER::SmnRead(ULONG SmnAddr)
{
    if (m_Bar5Va == NULL)
    {
        return 0xFFFFFFFF;
    }
    WRITE_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + 0x38), SmnAddr);
    (VOID)READ_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + 0x38));
    return READ_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + 0x3C));
}

VOID BASIC_DISPLAY_DRIVER::SmnWrite(ULONG SmnAddr, ULONG Value)
{
    if (m_Bar5Va == NULL)
    {
        return;
    ...
    WRITE_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + 0x38), SmnAddr);
    (VOID)READ_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + 0x38));
    WRITE_REGISTER_ULONG((PULONG)((PUCHAR)m_Bar5Va + 0x3C), Value);
}

//
// SMU v11.8 mailbox via SMN (Queue 0):
//   C2PMSG_66 (cmd)  = SMN 0x03B10A08
//   C2PMSG_82 (arg)  = SMN 0x03B10A48
//   C2PMSG_90 (ctrl) = SMN 0x03B10A68
// Protocol: clear ctrl if busy, write ARG, write CMD, poll ctrl==1, read ARG.
// Matches test-tools/bar5-smn-test.c (proven working).
//

#define BC250_C2PMSG_66 0x03B10A08
#define BC250_C2PMSG_82 0x03B10A48
#define BC250_C2PMSG_90 0x03B10A68

//
// SMU v11.8 Queue 3 (CPU/thermal/profile):
//   C2PMSG_66 (cmd)  = SMN 0x03B10A20
//   C2PMSG_82 (arg)  = SMN 0x03B10A88
//   C2PMSG_90 (ctrl) = SMN 0x03B10A80
//
#define BC250_Q3_C2PMSG_66 0x03B10A20
#define BC250_Q3_C2PMSG_82 0x03B10A88
#define BC250_Q3_C2PMSG_90 0x03B10A80

ULONG BASIC_DISPLAY_DRIVER::SmuQuery(USHORT Msg)
{
    return SmuQueryParam(Msg, 0);
}

ULONG BASIC_DISPLAY_DRIVER::SmuQueryParam(USHORT Msg, ULONG Param)
{
    return SmuSendQueue(0, Msg, Param);
}

//
// Universal SMU mailbox: Queue 0 = GFX freq/volt, Queue 3 = CPU/temp/profile.
// Queue select matches cyan-skillfish-governor and bar5-smn-test.c.
//
ULONG BASIC_DISPLAY_DRIVER::SmuSendQueue(ULONG Queue, USHORT Msg, ULONG Param)
{
    if (m_Bar5Va == NULL)
    {
        return 0xFFFFFFFF;
    ...
    ULONG cmdAddr, argAddr, rspAddr;

    if (Queue == 3)
    {
        cmdAddr = BC250_Q3_C2PMSG_66;
        argAddr = BC250_Q3_C2PMSG_82;
        rspAddr = BC250_Q3_C2PMSG_90;
    }
    else
    {
        cmdAddr = BC250_C2PMSG_66;
        argAddr = BC250_C2PMSG_82;
        rspAddr = BC250_C2PMSG_90;
    }

    ULONG Ctrl = SmnRead(rspAddr);
    if (Ctrl == 1)
    {
        SmnWrite(rspAddr, 0);
    }

    SmnWrite(argAddr, Param);       // ARG
    SmnWrite(cmdAddr, Msg);         // CMD

    for (ULONG i = 0; i < 500; i++)
    {
        if (SmnRead(rspAddr) == 1)
        {
            return SmnRead(argAddr);
        }
...
//
// CPU core unlock: SMU Queue 3 msg 0x98 (ungated SMN write) writes fixed
// 0x00FF to the passed SMN address. Core presence mask 0x0115A870:
// 0x77 (6 cores) -> 0xFF (8 cores / 16 threads). Volatile across cold boots.
//
#define BC250_SMU_Q3_MSG_UNLOCK_CORE 0x98
#define BC250_CPU_CORE_MASK_SMN       0x0115A870

NTSTATUS BASIC_DISPLAY_DRIVER::CoreUnlock(VOID)
{
    if (m_Bar5Va == NULL)
    {
        return STATUS_DEVICE_NOT_READY;
    ...
    ULONG mask = SmnRead(BC250_CPU_CORE_MASK_SMN);
    if (mask != 0x77)
    {
        DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL,
            "BC-250: CoreUnlock - mask 0x%08X not 0x77, skipping\n", mask);
        return STATUS_INVALID_PARAMETER;
    }

    ULONG rsp = SmuSendQueue(3, BC250_SMU_Q3_MSG_UNLOCK_CORE, BC250_CPU_CORE_MASK_SMN);
    DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL,
        "BC-250: CoreUnlock - msg 0x98 rsp=0x%08X, mask now 0x%08X\n",
        rsp, SmnRead(BC250_CPU_CORE_MASK_SMN));
                                                ...
                                                _In_    BOOLEAN            NonDestructiveOnly)
{
    ...
    switch (pChildStatus->Type)
    {
        case StatusConnection:
        {
            ...
            pChildStatus->HotPlug.Connected = IsDriverActive() && m_CurrentModes[pChildStatus->ChildUid].Flags.FrameBufferIsActive;
            return STATUS_SUCCESS;
        }
...
NTSTATUS BASIC_DISPLAY_DRIVER::PresentDisplayOnly(_In_ CONST DXGKARG_PRESENT_DISPLAYONLY* pPresentDisplayOnly)
{
    ...
    if (m_CurrentModes[pPresentDisplayOnly->VidPnSourceId].Flags.FrameBufferIsActive &&
        (m_CurrentModes[pPresentDisplayOnly->VidPnSourceId].FrameBuffer.Ptr != NULL))
    {
...
VOID BASIC_DISPLAY_DRIVER::BlackOutScreen(D3DDDI_VIDEO_PRESENT_SOURCE_ID SourceId)
{
    ...
    if (m_CurrentModes[SourceId].Flags.FrameBufferIsActive &&
        (m_CurrentModes[SourceId].FrameBuffer.Ptr != NULL))
    {
