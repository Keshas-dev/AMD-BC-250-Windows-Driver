...
int main() {
    ...
    printf("=== KIQ_SIZE write attempts (with KIQ active) ===\n\n");
    ...
    printf("KIQ_SIZE before active: 0x%08X\n", read_reg(0xE068));
    
    // Activate KIQ
    write_reg(0xE080, 0x00000001);
    printf("KIQ_ACTIVE: 0x%08X\n", read_reg(0xE080));
    
    // Try writing KIQ_SIZE while active
    uint32_t testValues[] = {0x00000001, 0x00000002, 0x00000004, 0x00000010, 
                             0x00000100, 0x00010000, 0x00020000, 0x00100000,
                             0x01000000, 0x10000000, 0x80000000, 0xFFFFFFFF};
    
    for (int i = 0; i < 12; i++) {
        write_reg(0xE068, testValues[i]);
        uint32_t readback = read_reg(0xE068);
        printf("  Wrote 0x%08X to KIQ_SIZE, read: 0x%08X %s\n", 
               testValues[i], readback, (readback == testValues[i]) ? "<-- CHANGED!" : "");
    }
    
    // Check RPTR
    printf("\nKIQ_RPTR after size writes: 0x%08X\n", read_reg(0xE06C));
