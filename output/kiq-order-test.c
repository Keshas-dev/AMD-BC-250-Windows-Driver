...
int test_order(uint32_t order) {
    uint32_t pa_low = 0x12345000;
    uint32_t rptr, wptr, active, scratch;
    ...
    rptr = read_reg(0xE06C);
    wptr = read_reg(0xE078);
    active = read_reg(0xE080);
    scratch = read_reg(0x32D4);
...
int main() {
    ...
    for (int order = 1; order <= 4; order++) {
        uint32_t scratch = test_order(order);
        printf("Order %d: SCRATCH = 0x%08X\n", order, scratch);
    }
