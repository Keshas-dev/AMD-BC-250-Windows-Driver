/* spi-pg-psp-only.c - write SPI_PG through PSP driver ONLY (PSP-specific IOCTLs) */
#include <windows.h>
#include <stdio.h>
#include <stdint.h>
#include "..\inc\amdbc250_ioctl.h"

/* PSP-specific IOCTLs (correct codes from PspIoctl.h) */
...
/* PSP driver expects these structs */
...
int main(void){
    ...
    /* Init PSP HW - PSP driver expects struct with PA + Size */
    ...
    DWORD rb;
    if(DeviceIoControl(hPsp,IOCTL_PSP_READ_REG,&readReq,sizeof(readReq),&readResp,sizeof(readResp),&rb,NULL)){
        printf("Baseline SPI_PG = 0x%08X\n",readResp.Value);
    } else {
        printf("PSP READ_REG failed err=%lu\n",GetLastError());
    }

    /* Write SPI_PG = 0x1F */
    PSP_WRITE_REQUEST writeReq = {0x5C3C, 0x1F};
    if(DeviceIoControl(hPsp,IOCTL_PSP_WRITE_REG,&writeReq,sizeof(writeReq),NULL,0,&br,NULL)){
        printf("Write SPI_PG=0x1F: OK\n");
    } else {
        printf("Write SPI_PG=0x1F: FAIL err=%lu\n",GetLastError());
    }
