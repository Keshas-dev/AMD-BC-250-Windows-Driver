@echo off
rem compile-dcn-targeted-probe.bat — build dcn-targeted-probe.exe
setlocal
...
cl /nologo /W3 /O1 /MT /Fe"%OUTDIR%\dcn-targeted-probe.exe" ^
    "%SRCDIR%dcn-targeted-probe.c" /link user32.lib kernel32.lib
if errorlevel 1 (
    echo BUILD FAILED
    exit /b 1
)
echo OK: %OUTDIR%\dcn-targeted-probe.exe
