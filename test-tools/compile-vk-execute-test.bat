@echo off
setlocal
:: compile-vk-execute-test.bat — Vulkan ICD real execute verification
set SRCDIR=%~dp0
set ODIR=%~dp0..\output
...
call "%VCVARS%" x64 >nul
cl /nologo /std:c11 /O2 /utf-8 /MD /W3 /wd4100 /wd4101 /wd4189 /wd4996 ^
  /I"%SDK%\Include\%SDKVER%\um" ^
  /I"%SDK%\Include\%SDKVER%\shared" ^
  /I"%SDK%\Include\%SDKVER%\ucrt" ^
  /Fe"%ODIR%\vk-execute-test.exe" ^
  "%SRCDIR%vk-execute-test.c" ^
  /link /libpath:"%SDK%\Lib\%SDKVER%\um\x64" /libpath:"%SDK%\Lib\%SDKVER%\ucrt\x64" ^
  kernel32.lib user32.lib
if errorlevel 1 (echo BUILD FAILED & exit /b 1) else (echo OK: output\vk-execute-test.exe)
