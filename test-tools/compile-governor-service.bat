@echo off
setlocal
:: compile-governor-service.bat — long-running GPU governor console service
set SRCDIR=%~dp0
set ODIR=%~dp0..\output
...
cl /nologo /O2 /utf-8 /MD /W3 /wd4100 /wd4101 /wd4189 /wd4996 ^
  ...
  /Fe"%ODIR%\governor-service.exe" ^
  "%SRCDIR%governor-service.c" ^
  /link /libpath:"%SDK%\Lib\%SDKVER%\um\x64" /libpath:"%SDK%\Lib\%SDKVER%\ucrt\x64" ^
  kernel32.lib user32.lib
if errorlevel 1 (echo BUILD FAILED & exit /b 1) else (echo OK: output\governor-service.exe)
