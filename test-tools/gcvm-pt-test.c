...
int main(void) {
    ...
    /* If RPTR stuck at 0, try manual kick via GPU driver WPTR write */
    if (kiqRptr == 0) {
        printf("\n--- RPTR still 0 — diagnostic reads ---\n");
        printf("  ME_CNTL      (0x4A74) = 0x%08X\n", R(0x4A74));
        ...
        printf("  KIQ_WPTR     (0xE078) = 0x%08X\n", R(0xE078));
        printf("  KIQ_RPTR     (0xE06C) = 0x%08X\n", R(0xE06C));
        printf("  CP_RB_BAS_ADDR (0xDA60) = 0x%08X\n", R(0xDA60));
        
        printf("\n--- RPTR still 0 — kicking CP via direct WPTR write ---\n");
        /* Write WPTR to hardware to signal CP (bit 31 = wake-up) */
        UCHAR kick[8] = {0};
        *(ULONG*)(kick+0) = 0xE078;
        *(ULONG*)(kick+4) = kiqWptr | 0x80000000; /* WPTR with wake bit */
        DWORD br = 0;
        DeviceIoControl(hGpu, IOCTL_GPU_WRITE, kick, 8, NULL, 0, &br, NULL);
        Sleep(100);
        ULONG rptrAfter = R(0xE06C);
        ULONG wptrAfter = R(0xE078);
        printf("  After kick: WPTR=0x%08X RPTR=0x%08X\n", wptrAfter, rptrAfter);
        if (rptrAfter > 0) {
            printf("  >>> RPTR ADVANCED! CP processed the ring!\n");
        }

        /* Try direct KIQ_WPTR write with bit 0 = queue enable */
        printf("\n--- KIQ queue enable attempt ---\n");
UCHAR enable[8] = {0};
        *(ULONG*)(enable+0) = 0xE078;
        *(ULONG*)(enable+4) = 1; /* WPTR = 1 */
        DeviceIoControl(hGpu, IOCTL_GPU_WRITE, enable, 8, NULL, 0, &br, NULL);
        Sleep(50);
        rptrAfter = R(0xE06C);
        wptrAfter = R(0xE078);
        printf("  After WPTR=1 write: WPTR=0x%08X RPTR=0x%08X\n", wptrAfter, rptrAfter);
    }
